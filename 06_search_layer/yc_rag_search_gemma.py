"""
yc_rag_search_gemma.py
----------------------
RAG-based Property Search Module — PropertyLens Layer 06 (Ollama Gemma 3 variant).

Implements a 3-stage pipeline on top of the existing Neo4j weighted search:

  Stage 1 — NL → Search Params:
    Ollama Gemma 3 extracts { weights, filters, special_query_type } as a validated
  ...
  Stage 3 — Results → NL Answer:
    Ollama Gemma 3 summarises the top-k property results into a plain-English answer.

Requirements
------------
  - Ollama daemon running locally (default: http://localhost:11434)
  - Gemma 3 model pulled: ollama pull gemma3 (or gemma2:27b, etc.)
...
-----
    ...
    # Option A: let the class load Gemma 3 internally via Ollama
    with PropertyRAGSearchGemma() as rag:
        result = rag.ask("Find a 4-room flat near MRT in Bishan under $900k")
        print(result["answer"])
        print(result["results"])

    # Option B: pass a pre-loaded LLM instance
    from langchain_community.llms import Ollama
    local_llm = Ollama(model="gemma3")
    with PropertyRAGSearchGemma(llm=local_llm) as rag:
        result = rag.ask("Quiet large flat with good lease left")
        print(result["answer"])
...
# ---------------------------------------------------------------------------
# Prompt templates (Gemma 3 instruction-tuned format)
# ---------------------------------------------------------------------------
...
def _build_gemma_prompt(system: str, user: str) -> str:
    """Format a prompt using Gemma 3's instruction-tuned format."""
    return (
        f"[INST] {system.strip()}\n\n{user.strip()} [/INST]"
    )
...
# ---------------------------------------------------------------------------
# Ollama LLM wrapper (if langchain_community is not available)
# ---------------------------------------------------------------------------

class SimpleOllamaLLM:
    """
    Minimal Ollama LLM wrapper using requests.
    Falls back if langchain_community.llms.Ollama is not available.
    """
    def __init__(
        self,
    ...
    def invoke(self, prompt: str) -> str:
        ...
        try:
            ...
            raise ImportError("requests library is required for SimpleOllamaLLM")
        ...
        try:
            ...
            raise RuntimeError(
                f"Ollama API error at {url}: {exc}. "
                "Ensure Ollama daemon is running and model is pulled."
            ) from exc
...
class PropertyRAGSearchGemma:
    """
    RAG-based property search using Ollama Gemma 3 and Neo4j.

    Pipeline
    --------
    1. Stage 1 — NL → JSON params  (Gemma 3 parameter extraction)
    2. Stage 2 — JSON → Results    (existing Neo4jPropertySearch)
    3. Stage 3 — Results → Answer  (Gemma 3 NL generation)
    ...
    model_name : str
        Ollama model name. Default: "gemma3"
        Alternatives: "gemma2:27b", "gemma:13b-instruct", etc.
    max_new_tokens : int
        Max tokens for LLM generation stages. Default: 512.
    temperature, top_p, top_k :
        Sampling parameters for Ollama.
    top_k_results : int
        Default number of properties to retrieve per query. Default: 5.
    llm : optional
        Pre-loaded LLM instance (e.g. langchain_community.llms.Ollama).
        If provided, skips internal Ollama loading.
    """
    ...
    def __init__(
        ...
        # LLM — reuse passed instance or load Gemma 3 via Ollama
        if llm is not None:
            self._llm = llm
        else:
            self._load_llm()
    ...
    def _load_llm(self) -> None:
        """Load Gemma 3 via Ollama HTTP API."""
        # Try langchain_community.llms.Ollama first
        try:
            from langchain_community.llms import Ollama
            print(f"Loading {self._model_name} via Ollama at {self._ollama_base_url} ...")
            self._llm = Ollama(
            ...
            print(f"{self._model_name} loaded.")
            return
        except ImportError:
            print("langchain_community not found; using SimpleOllamaLLM")

        # Fallback to SimpleOllamaLLM
        print(f"Loading {self._model_name} via Ollama at {self._ollama_base_url} ...")
        self._llm = SimpleOllamaLLM(
            base_url=self._ollama_base_url,
            model=self._model_name,
            temperature=self._temperature,
            top_p=self._top_p,
    ...
    def _stage1_extract_params(
        ...
        """
        Use Gemma 3 to extract search parameters from a natural-language query.
        ...
        prompt = _build_gemma_prompt(_STAGE1_SYSTEM, user_content)
        ...
        # Extract JSON substring (Gemma may include explanations)
    ...
    def _heuristic_extract_params(self, query: str) -> dict[str, Any]:
        ...
        Gemma 3 fails to emit valid JSON.
        """
        params = _fresh_default_params()
        params["weights"] = {}
        params["filters"] = {}
    ...
    def _stage3_generate_answer(
        ...
        prompt = _build_gemma_prompt(_STAGE3_SYSTEM, user_content)
        ...
        # Gemma 3 output may be longer; extract the main answer
        answer = raw_output.strip()

        # Sanity checks for nonsensical outputs
        answer_lower = answer.lower()
        if (
            not answer
            or "[/inst]" in answer_lower
            or "<|system|>" in answer
    ...
    def __enter__(self) -> "PropertyRAGSearchGemma":
        return self
    ...
    def __repr__(self) -> str:
        return (
            f"PropertyRAGSearchGemma("
            f"model={self._model_name!r}, "
            f"ollama_url={self._ollama_base_url!r}, "
