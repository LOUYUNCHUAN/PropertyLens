# PropertyLens — Claude Instructions
...
**First time setup:**
1. **Step 0:** `00_download_data_from_HF.ipynb` — Download feature snapshots from Hugging Face (run from **repo root**)
   - Requires: `HF_TOKEN` in `.env`
   - Populates: `hf_data/02_feature_layer/training/outputs/` with latest feature tables

2. **Step 1** (optional): Refresh raw data via `01_data_layer/pipelines/`
   - `raw_data_collection.ipynb` — OneMap geocoding, POI enrichment
   - `raw_data_sanity_check.ipynb` — Validation, deduplication, lease imputation

3. **Step 2** (optional): Rebuild features via `02_feature_layer/training/`
   - `FeatureDealing.ipynb` — Engineering pipeline
   - `FeatureValidation.ipynb` — Quality diagnostics

4. **Step 3:** Train models via `03_ml_layer_hybrid/` **in order**:
   - `01_xgb_lgb_ensemble.ipynb` — XGB + LGB baseline
   - `02_hybrid_ensemble.ipynb` — K-Means cluster routing, stacked models
   - `03_hybrid_prediction.ipynb` — Inference, bulk evaluation

5. **Step 4:** Train XAI artefacts via `04_xai_layer/` **in order**:
   - `04_hybrid_xai_train.ipynb` — Train surrogate models, Apriori rules
   - `05_hybrid_xai_explain.ipynb` — Per-listing attributions

6. **Step 5** (optional): Photo-adjusted predictions via `05_photo_layer/` **in order**:
...
### Latest-Snapshot Loading Pattern
Always discover artefacts dynamically — never hard-code a date:
```python
import glob
from pathlib import Path
feature_dir = REPO_ROOT / 'hf_data/02_feature_layer/training/outputs'
latest = sorted(feature_dir.glob('hdb_feature_table_*.csv'))[-1]
```
...
- **Backup file globbing:** Always filter out `*_backup_*.csv` files when globbing HDB CSVs.
...
See layer-specific `CLAUDE.md` files for deeper workflows:
- [01_data_layer/CLAUDE.md](01_data_layer/CLAUDE.md) — Raw data collection & validation
- [02_feature_layer/CLAUDE.md](02_feature_layer/CLAUDE.md) — Feature engineering & export
- [03_ml_layer_hybrid/CLAUDE.md](03_ml_layer_hybrid/CLAUDE.md) — Ensemble training & inference
- [04_xai_layer/CLAUDE.md](04_xai_layer/CLAUDE.md) — Explainability artefacts
- [05_photo_layer/CLAUDE.md](05_photo_layer/CLAUDE.md) — Photo condition scoring
...
- Hard-code artefact dates; use **latest-snapshot glob logic** instead
- Create Python virtual environments (ask the user which environment to use if you need to run code)

**DO:**
- Match the surrounding file's style (imports, spacing, comments)
- Use **date-stamped filenames** for all new artefacts
- Cross-reference layer `CLAUDE.md` files when changing artefact paths
- Verify paths from both **repo root** and **layer subdirectory** cwd
- Log key metrics and sample outputs in notebook cells
- Ask the user which Python virtual environment to use before running code locally
- Check compatibility between macOS and any target deployment environment before running code locally; resolve compatibility issues before proceeding
